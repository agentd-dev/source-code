...
| **Node kinds** | `read_file` `read_env` `read_mcp_resource` `parse_json` · `template_render` `json_select` `diff_compute` · `llm_infer` `agent_loop` · `write_file` `create_dir` `http_request` `call_mcp_tool` `shell_run` · `call` (sub-workflow) · `parallel` (concurrent fan-out) · `map` (bounded fan-out over a list) · `condition` `switch` `merge` `fail` `pause_for_approval` `respond` (shape the webhook reply) `terminate` |
| **Durable & HITL** | `pause_for_approval` node checkpoints under `--state-dir` and stops with a `paused` outcome (exit 7); `--resume RUN_ID` continues with state restored. `--checkpoint-each-node` snapshots after every node so a crashed run is recoverable (`--resume-incomplete` / `--list-checkpoints`) — the line between automation and an agent that works alongside you |
...
| **Observability** | Structured spans (`workflow.run` → `node.execute`), Prometheus `/metrics` (incl. llm calls + tokens), `/healthz`, dedicated audit JSONL sink with redaction, W3C traceparent propagation in and out, optional OTLP gRPC export, structured run records (`--record`) with an `agentd inspect` timeline |
