...
| **Node kinds** | `read_file` `read_env` `read_mcp_resource` `parse_json` · `template_render` `json_select` `diff_compute` · `llm_infer` `agent_loop` · `write_file` `create_dir` `http_request` `call_mcp_tool` `shell_run` · `call` (sub-workflow) · `parallel` (concurrent fan-out) · `condition` `switch` `merge` `fail` `pause_for_approval` `respond` (shape the webhook reply) `terminate` |
