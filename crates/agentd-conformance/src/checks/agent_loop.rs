...
pub fn checks() -> Vec<Check> {
    vec![
        Check { id: "agent-loop/direct-final-answer", category: Category::AgentLoop,
            desc: "an LLM that answers immediately completes in one step", run: direct_final_answer },
        Check { id: "agent-loop/tool-call-and-result", category: Category::AgentLoop,
            desc: "the agent executes an LLM tool call and feeds the result back", run: tool_call_and_result },
        Check { id: "agent-loop/multi-step-converges", category: Category::AgentLoop,
            desc: "a tool-call turn + an answer turn converge on the post-tool result", run: multi_step_converges },
        Check { id: "agent-loop/budget-bounds-the-loop", category: Category::AgentLoop,
            desc: "--max-steps cuts off a non-converging loop → exit 7, exhausted_steps", run: budget_bounds_the_loop },
    ]
}
...
fn run_loop(h: &Harness, script: &str, with_mcp: bool) -> (RunResult, crate::harness::MockLlm) {
    ...
    if with_mcp {
        args.push("--mcp".into());
        args.push(format!("{} --no-emit", h.mock_mcp_spec("mock", "file:///in.json")));
    }
...
fn direct_final_answer(h: &Harness) -> Outcome {
    ...
    Outcome::require(r.code == Some(0), format!("want exit 0, got {:?}; stderr:\n{}", r.code, r.stderr))
        .and(|| Outcome::require(!r.stdout.trim().is_empty(), "no final answer printed to stdout".to_string()))
        .and(|| {
            let f = final_event(&r);
            Outcome::require(
                f.as_ref().map(|f| f["status"] == "completed").unwrap_or(false),
...
fn tool_call_and_result(h: &Harness) -> Outcome {
    ...
    if r.code != Some(0) {
        return Outcome::fail(format!("want exit 0, got {:?}; stderr:\n{}", r.code, r.stderr));
    }
    let events = r.events();
    let called = events.iter().any(|e| e["event"] == "tool.call" && e["tool"] == "resource.read");
    if !called {
    ...
    Outcome::require(result_ok, "no successful tool.result was fed back".to_string())
}
...
fn multi_step_converges(h: &Harness) -> Outcome {
    ...
    let steps = final_event(&r).and_then(|f| f["steps"].as_u64()).unwrap_or(0);
    Outcome::require(steps >= 2, format!("expected ≥2 steps (tool + answer), saw {steps}")).and(|| {
        Outcome::require(
            r.stdout.contains("read complete"),
            format!("final answer didn't reflect the post-tool result; stdout: {:?}", r.stdout.trim()),
        )
    })
}
...
fn budget_bounds_the_loop(h: &Harness) -> Outcome {
    ...
    Outcome::require(r.code == Some(7), format!("want exit 7 (budget), got {:?}; stderr:\n{}", r.code, r.stderr)).and(|| {
        Outcome::require(
            r.events().iter().any(|e| e["event"] == "loop.final" && e["status"] == "exhausted_steps"),
