...
- **Post-completion (operator asked to work the deferred scope):** **done so far
  ...
  **shipped served `subagent.send`** — the last substantial deferred capability
  ...
  M5 checklist item for detail. **Still deferred:** child resource *limits*
  (`memory.max`/`pids.max`) need controller delegation (`subtree_control` →
  `EBUSY` here — a deployment concern, not in-binary); a standalone
  `agentd-conformance` crate (non-essential reorg + external reference server).
