...
- **Post-completion (operator asked to work the deferred scope):** **done so far
  ...
  **completion-as-self-resource** — `resource.read agentd://subagent/<handle>`
  ...
  `resources` capability. **Still deferred:** served `subagent.send`/`cancel`/
  `status` + per-served-subagent `agentd://subagent/<handle>` (need a tracked
  served-async registry) + `resources/subscribe` (push); self-subscribe → auto
...
- [~] self-MCP **server** listener (`mcp/server.rs`, `--serve-mcp unix:`) — **transport + the `subagent.spawn` action tool + the `agentd://` read-only resource surface landed** (`resources/list` + `resources/read` over `agentd://status`; see M4/M5). _Remaining: `subagent.send`/`cancel`/`status` + per-served-subagent `agentd://subagent/<handle>` resources (need a tracked served-async-spawn registry) + `resources/subscribe` (push notifications)._
