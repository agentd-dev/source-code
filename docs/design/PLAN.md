...
- **Post-completion (operator asked to work the deferred scope):** **done so far
  ...
  **Still deferred:** cgroup `cgroup.kill`/`memory.high` (infra-gated — no writable
  cgroup-v2 here); a standalone `agentd-conformance` crate (non-essential reorg +
  external reference server).
