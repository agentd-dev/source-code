...
- **Observability is a first-class, cross-cutting requirement** (operator
  ask): the agentd AND every subagent must be observable / traceable / logged /
...
- **Post-completion (operator asked to work the deferred scope):** **done so far
  ...
  **completion-as-self-resource** — `resource.read agent://subagent/<handle>`
  ...
  `resources/read` expose `agent://status` (this agentd's live state) with the
  ...
  **wired self-subscribe → a warm continue route** (the signature capability — a
  self-subscribing agentd re-enters one live session per event, not a fresh spawn;
  ...
  **shipped served `subagent.send`** — the last substantial deferred capability
  ...
  {handle,message}` injects the next user message so the agentd runs another turn
  ...
  knows agentd only as a binary + the MCP/JSON-RPC spec — never linking the lib,
...
- [x] `agentd-conformance` MCP client+server conformance + supervisor behavior — **shipped as a standalone, black-box crate** (`crates/agentd-conformance`, back in the workspace; dev-deps `serde_json` + `libc` only, never links the agentd lib). It builds the real binary on demand and runs **31 checks across 5 families**: `mcp-server` (served self-MCP — initialize, tools/list, tools/call, resources, ping, error codes -32601/-32602/-32002, notification + malformed-input survival), `mcp-client` (a recording `confmcp` reference server captures agentd's client handshake → `notifications/initialized` → tools/list discovery → resource subscribe), `supervisor` (exit-code table 0/2/4/6 + SIGTERM→exit-0 drain), `agent-loop` (the ReAct loop end-to-end: direct answer; tool call executed + fed back; multi-step convergence; `--max-steps` bounding → exit 7), `security` (lethal-trifecta refusal + `--allow-trifecta` override + token non-leak). Runs as `cargo test -p agentd-conformance` (one test per family) or `cargo run -p agentd-conformance [--json]` (PASS/FAIL report). The behaviours remain *also* covered by the agentd `tests/` e2e suite; this crate is the consolidated, reportable spec. (The retired pre-rewrite conformance crate — a DAG/`http_request`-node corpus — was removed.)
