...
- **Post-completion (operator asked to work the deferred scope):** **done so far
  ...
  **liveness env knobs** + the **live stuck→kill chaos test** (`chaos_e2e.rs`:
  ...
  (`tests/warm_session.rs` + `tests/subagent_spawn.rs`). **Still deferred:** async
  `subagent.spawn{async,detach}` + completion-as-self-resource; served
  `subagent.send`/`cancel`/`status` + `agentd://` resources (warm sessions now
  exist — these need a served session-addressing scheme on top); self-subscribe →
  auto `continue(this_session)` route; cgroup `cgroup.kill`/`memory.high`
