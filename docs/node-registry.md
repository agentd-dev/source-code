...
| `a2a` | — | `command` `roles` `schema` `into` `inputs` | Fires when a principal sends a message whose command matches. Declaring `command` REGISTERS it as an A2A command the listener accepts; `schema` is the payload CONTRACT — a non-conforming command is refused at the listener, synchronously, naming the mismatch. `roles` narrows who may fire it. Output: `…output.args.*` (the typed payload), plus `parts`/`text`/`principal`. **`into: {stream, subject}`** appends the message to a stream instead of firing a run, after the principal and any `roles` filter have been applied — the peer-facing half of the same binding. |
| `webhook` | `path` | `methods` `auth` `parallelism` `on_overflow` `rate` `idempotency` `respond` `filter` `signal` `into` `inputs` | Fires on an inbound HTTP request at `path`. Needs `webhooks.listen`; a non-loopback listener must authenticate every route. `rate: "<burst>/<per>s"` throttles arrivals (429 + Retry-After past it). `signal: "name/{{ body.field }}"` also fires that signal with the payload — the webhook→signal relay as one field. **`into: {stream, subject}`** appends the request to a durable stream instead of firing a run (RFC 0035 §5) — verification, filtering and rate limiting still run first, so it is a different destination for an accepted request, not a way past the gate. That is what gives a webhook replay-after-downtime: a run fired at request time is lost if nothing was ready for it, while an appended event waits for a consumer that may not exist yet. Answers `202 {"status":"appended"}`, or `503` if the append is refused — never a 202 over a dropped event. Incompatible with `respond: sync`, which has no run to wait for. |
...
| `emit` | — | `stream` `subject` `data` `correlation` `forward` `note` `audit` `metric` `value` | With `stream`/`subject` (they travel together, or a load error): publishes an event to a declared stream that any workflow's `stream` start can consume — durable, replayable, exactly-once downstream (the event id is the step's idempotency key, so a crash-replayed emit lands under the same id and consumers drop the copy). Without them: writes `note` to the root transcript, logs `audit` as an audit record, and returns `value` as the step output. **`forward: {webhook: <url>, allow_private?}`** or **`forward: {peer: <name>}`** pushes the event outward as it appends (RFC 0035 §5): the durable copy is the source of truth and the push is only the notification, so a failed forward is logged and does not fail the step. A peer receives it as a `stream.forwarded` command, which its own `a2a` start can bind back onto a stream with `into:` — one instance's `emit` becoming another's stream event. Outbound dials go through the same SSRF/egress guard as the `http` node; reaching a private address is a per-node `allow_private` opt-in. One destination, not both. |
