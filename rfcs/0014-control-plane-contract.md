...
```jsonc
{
  "contract_version": "1.0",            // the agentctl↔agent contract major.minor (§3.4)
  "agent_version": "2.1.0",            // CARGO_PKG_VERSION
...
- **`contract_version`** (major.minor) is the overall agentctl↔agent contract.
...
| Env | Source | Used by |
|---|---|---|
| `AGENT_POD_NAME` | `metadata.name` | identity (0015) |
| `AGENT_POD_UID` | `metadata.uid` | identity |
| `AGENT_POD_NAMESPACE` | `metadata.namespace` | identity |
| `AGENT_NODE_NAME` | `spec.nodeName` | identity |
| `AGENT_POD_GRACE_SECONDS` | `= terminationGracePeriodSeconds` | drain (RFC 0011) |
| `AGENT_SHARD` | `"K/N"` (from the StatefulSet ordinal) | sharding (0019) |

Per-endpoint intelligence credentials use one keying scheme across 0017/0018:
`AGENT_INTELLIGENCE_TOKEN` (≡ endpoint 1), then `AGENT_INTELLIGENCE_TOKEN_2`,
