...
2. **Claiming is opt-in per route via `disposition: claim`** (an extension of the
   ...
   default the **`AGENTD_RUN_ID`** (RFC 0011 §6) the worker would have used anyway.
...
3. **`--shard K/N` statically partitions the key space.** An instance with shard
   ...
   `agentd_shard_skipped_total`). The hash is **stable across versions and
...
5. **agentd exposes autoscaling signals; agentctl/KEDA owns the scaler.** The
   signal set — `agentd_reactive_backlog`, `agentd_active_subagents`,
   `agentd_saturation`, `agentd_tokens_per_sec`, `agentd_intelligence_latency_ms`
   — is part of the **frozen metrics schema (RFC 0016)**, surfaced on the existing
   metrics endpoint (RFC 0010) and mirrored as `agentd://metrics` /
   `agentd://capacity` resources (RFC 0005). The intended scaler behaviour (scale
...
6. **Standby (`--standby`) is a reactive worker that holds connections warm and
   ...
   intelligence + MCP connections, `agentd_ready=1`) for near-zero cold-start when
...
{ "jsonrpc":"2.0","id":7,"method":"tools/call","params":{
    ...
    "_meta":{
      "agentd/claim_key":"01J8Z…",        // = RUN_ID (RFC 0011 §6); the dedupe key
      "agentd/instance":"pod-abc",          // identity (RFC 0014 §5 downward API)
      "agentd/shard":"3/8",                 // for server-side observability only
...
// work.ack — work done; the side effect, keyed on agentd/claim_key, is committed.
{ "name":"work.ack", "arguments":{ "lease_id":"L-991" },
  "_meta":{ "agentd/claim_key":"01J8Z…" } }
...
- The **claim key is the `RUN_ID`** (RFC 0011 §6.1). agentd injects
  `agentd/run_id` (== `agentd/claim_key`) into the `_meta` of *every* downstream
...
- `work.ack` carrying `agentd/claim_key` lets the server **collapse the
...
| Lease TTL | `--claim-ttl` / `AGENTD_CLAIM_TTL` | `30s` | server is the authority; this is the *requested* TTL |
| Renew fraction | `AGENTD_CLAIM_RENEW_FRACTION` | `0.33` | heartbeat at `ttl * fraction` |
...
{ "AGENTD_SHARD": "3/8" }   // from: K = ordinal(pod-3), N = .spec.replicas
```
...
agentctl: kubectl scale / patch .spec.replicas 8 → 12   (the StatefulSet)
  for each pod, rolling:
    SIGTERM pod-k  ──► drain (RFC 0011 §4 + §6 below):
                        ...
                        4. exit 0
    start pod-k with AGENTD_SHARD="k/12"  ──► new owns() predicate
```
...
| Knob | Flag / env | Default | Notes |
|---|---|---|---|
| Shard identity | `--shard K/N` / `AGENTD_SHARD` | `0/1` (single shard) | `0 <= K < N`; validated exit 2 |
| Shard key (per route) | `--route …,shard_key=<expr>` | resource URI | stable item key; co-locates related items |
| Timer shard behaviour | `AGENTD_SHARD_TIMER` | `shard0` | `shard0` (one ticker) \| `keyed` (shard on target) |
...
`agentd://metrics` and `agentd://capacity` resources (RFC 0005 §3.3) for
vsock-only fleets with no cluster network (RFC 0014 §2).

| Signal | Type | Meaning | Scaler intent |
|---|---|---|---|
| `agentd_reactive_backlog` | gauge | distinct queued+unclaimed items this replica sees (sum of route `queue` depths + offered-not-yet-claimed) | **primary scale-up trigger**: backlog > 0 sustained ⇒ add replicas |
| `agentd_active_subagents` | gauge | live subagents (= RFC 0010 `agentd_active_subagents`) | concurrency headroom; near `max` ⇒ saturated |
| `agentd_saturation` | gauge `[0.0,1.0]` | `in_flight / capacity` where capacity = `min(max_inflight·routes, max_total_subagents)` | scale up as it approaches 1.0; the HPA "utilization" target |
| `agentd_tokens_per_sec` | gauge | rolling tokens/s across the tree (derived from RFC 0010 `agentd_tokens_total`) | intelligence-bound load; informs *model-aware* placement & cost ceilings |
| `agentd_intelligence_latency_ms` | gauge | p50/p95 intel call latency (from RFC 0010 `gen_ai.client.operation.duration`) | rising latency ⇒ upstream saturation; **scale the model service, not necessarily agentd** |
| `agentd_claims_lost_total` | counter | claims lost to another replica (contention) | high & rising under low backlog ⇒ **over-provisioned**, scale *down* |
| `agentd_shard_skipped_total` | counter | items dropped as out-of-shard | sanity: confirms shard partitioning is live |

`agentd_reactive_backlog` is new-to-this-RFC in intent but mechanically a sum
...
- **Scale up on backlog, not on CPU.** A reactive worker is intelligence-bound
  ...
  `agentd_reactive_backlog` (a `prometheus` scaler) or `agentd_saturation` as an
...
- **Respect drain on scale-down (Decision 7).** KEDA/HPA scale-down deletes a
  ...
  The HPA's `terminationGracePeriodSeconds` MUST exceed `AGENTD_DRAIN_TIMEOUT`
...
- **Scale-down preference: lame-duck the least-loaded.** agentctl picks the
  victim (lowest `agentd_active_subagents`, drained via the `lame-duck` /
  `drain` management tools, RFC 0014 §4 / RFC 0015). agentd exposes per-pod load;
  the *choice* is agentctl's. `agentd_claims_lost_total` rising under low backlog
...
| Readiness | `agentd_ready=1` (RFC 0010) | `agentd_ready=1`, **and** `agentd://capacity` advertises free slots |
...
2. **Directed-assign (push, via the management surface).** agentctl, having
   ...
   0014 §3); agentd only exposes the spawn/assign primitive and `agentd://capacity`
   so agentctl can see who is free.

```jsonc
// agentd://capacity  (RFC 0005 resource) — what agentctl reads to place work
...
| Knob | Flag / env | Default | Notes |
|---|---|---|---|
| Standby mode | `--standby` / `AGENTD_STANDBY` | off | reactive worker, warm, assignment-driven |
| Assignment channel | `--assign-from server:uri` | — | the shared pending resource (claim-pull) |
| Warm intelligence | `AGENTD_WARM_INTEL` | `on` (when `--standby`) | keep the intel session open while idle |
...
| 11 | **Standby member assigned work, then dies before claiming** | Claim-pull: nothing was claimed → no loss, another member wins. Directed-assign: the directed subscribe produced no claim → the assigner observes no `agentd://capacity` change / no ack and re-assigns (agentctl policy). |
| 12 | **All replicas in a shard are over budget (tree token ceiling)** | Each stops spawning and drains warm sessions (RFC 0008 §3.1, the ultimate backpressure), releasing claims (§6). Backlog rises → `agentd_reactive_backlog` climbs → the scaler adds replicas (which have fresh tree budgets). Budget exhaustion becomes a *scale signal*, not a meltdown. |
...
`claim`; assemble a `StatefulSet` whose ordinals feed `AGENTD_SHARD`; target the
...
- **RFC 0005 (self-MCP & control protocol):** standby directed-assign reuses
  `subagent.spawn`/`subagent.send` and the `agentd://capacity`/`agentd://metrics`
...
- **Shard injection key name.** §4.2 assumes agentctl injects `AGENTD_SHARD="K/N"`
  from the StatefulSet ordinal, paralleling RFC 0011 §3.3's
  `AGENTD_POD_GRACE_SECONDS` downward-API convention. The exact env name is a
...
- **RFC 0005** — self-MCP server & control protocol: `subagent.spawn`/`send`, the
  `agentd://capacity`/`agentd://metrics` resources used for placement and signals.
