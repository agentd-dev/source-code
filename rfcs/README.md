...
| [0032](0032-interface-and-observation-plane.md) | The display-client interface & observation plane | Implemented | `interface.enabled/debug/origins`; the global `SubscribeToEvents` feed (seq-cursored ring, principal-scoped, hello/event/goodbye); taskless reads (`interface.info`, `conversation.get`, `run.get`, `debug.events`); browser CORS via allowed origins; the `agentd tui`/`agentd ui` passthrough; the `interface/` Node clients (`@agentd/client`, `@agentd/tui`, `@agentd/ui`). |
