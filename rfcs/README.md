...
| [0021](0021-durable-workflows-and-parity-extensions.md) | Durable workflows & parity extensions | Draft (workflow track) | Workflow dialect 2: write reducers (`writes_mode`), the in-process `parallel` node, the `human` gate (served payload + A2A `input-required` + reply wait), and the **MCP checkpointer** (per-superstep durable state via a 3-tool MCP profile; `--workflow-resume`, fork/time-travel, budget carry-over) — plus fail-closed dialect hygiene (strict node fields). |
